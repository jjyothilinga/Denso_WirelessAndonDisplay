...
void COM_txCMD( UINT8 deviceAddress,UINT8 cmd, UINT8 data , UINT8 length )
{
	UINT8 cmdPacket[10] = {0};
	UINT8 i,j,cs;
	...
	cmdPacket[i++] = data;
 	...
 	cs = checksum(&cmdPacket[1], i );
	while((cs == CMD_SOP ) || (cs == CMD_EOP)) //if check sum matches sop or eop
	{
		cmdPacket[1]++; 						// change length
		cs = checksum(&cmdPacket[1], i ); 	  //recalculate check sum
	}
